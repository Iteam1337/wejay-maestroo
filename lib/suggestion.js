...
module.exports = {
  next : function(users, startDate, stopDate, done){
    ...
    var userIds = users.map(function(user){ return user.facebookId});
    ...
    connection.query([
      ...
      '  )',
      'LIMIT 0,1;'
    ].join('\r\n'), {userIds: userIds.join(','), stopDate: moment(stopDate).toISOString(), startDate: moment(startDate).toISOString()})
